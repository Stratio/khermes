...
object ResourcesHelper extends HermesSerializer {
  ...
  def getResources(name: String): Seq[String] = Try(
    ...
    case Failure(_) => throw new HermesException(s"Error loading invalid name /locales/$name")
  }

  def getResource(name: String, file: String): InputStream = Option(
    getClass.getResourceAsStream(s"/locales/$name/$file")).getOrElse(
    throw new HermesException(s"Error loading invalid resource /locales/$name/$file"))
}
