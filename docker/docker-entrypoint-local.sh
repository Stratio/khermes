...
if [ $SEED = "true" ]; then
    ...
    java -jar -Dkhermes.client=true -Dkhermes.ws=false -Dakka.remote.netty.tcp.port=$NODE_PORT -Dakka.cluster.seed-nodes.0=akka.tcp://khermes@localhost:$SEED_PORT -Dzookeeper.connection=master.mesos:$ZK_PORT /khermes.jar
fi
